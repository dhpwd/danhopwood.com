...
The thing that makes one setup look impossible and another look manageable isn't complexity. It's maturity. Two kinds: the maturity of your context (the information and instructions the agent reads), and the maturity of your workflows (the skills and agents that do the repeatable work).

When you start, there are inaccuracies, contradictions and gaps everywhere. That's just the process. You catch and fix those things over months, and there's no real way to shortcut it. Which is exactly why the advice everyone gives is the right advice: start simple, don't overthink the structure, build it up over time. The structure is the thing you arrive at, not the thing you design.
...
**A persona.** Claude Code (my tool of choice) assumes by default that you want to write code. Most of the time I don't. The useful part is that you can [replace the default identity entirely](/posts/two-ways-to-change-claudes-personality), not just [add instructions on top of it](/posts/cowork-vs-claude-code). So mine is a persona I call Emma, an executive assistant briefed on the three things I care about: simplicity, honesty and brevity. That governs [how she talks to me](/posts/be-concise-is-the-wrong-instruction), but the bigger effect is on what she writes. Every document the agent produces inherits that bar, which is the difference between a vault you actually use and a vault full of bloated documents no one reads. It's one instruction set that compounds into everything.

![Emma introducing herself in a fresh session – an executive assistant, not a coding assistant](../../assets/images/cc-emma-intro.png)
...
**Two agents and a document.** Last week I wanted slides for a talk to match my website. I pointed Claude Design at my site and it built a reusable design system. Then I had it write a plain summary of the templates it had created – the constraints, the limits, what each template was for. That document became the brief for Claude _Code_, which had the talk content and wrote the slides against those constraints. Neither agent could have done it alone. The interesting part is the bridging document, which now lives in my vault, so the next talk skips the whole process. Capture once, reuse forever. Do enough of it and you stop doing the work yourself, and start [designing the systems that do it for you](/posts/writing-loops).

## The one question that builds the whole thing

People want to know how you get from an empty folder to that. The answer is a question, asked over and over.
...
This adds friction early. You trip over these moments constantly in the first few months and each one slows you down. But every time you answer the question, you've turned a one-off annoyance into a permanent asset. That friction is you building the thing.

## The habit to copy first

If you take one habit from this, take this one. It works on day one, regardless of how mature anything else is.
...
The place to begin is one document about your company. Anything. It's the seed the structure grows from.
