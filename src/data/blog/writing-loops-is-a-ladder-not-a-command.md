...
That interpretation is wrong. `/loop` is the smallest, least powerful version of what he's describing. 'Writing loops' isn't a command you run. It's a level you climb to.
...
There's a ladder here, and each rung hands more of the 'what do I do next' decision to the machine.

1. **A skill on a loop.** `/loop` re-fires a prompt or a skill. I still wrote the prompt, so the loop only repeats it. This is the bottom rung, where most people stop
2. **A standing instruction.** I write one instruction to a file once, and the agent reads it at the start of each pass and decides what to do. Properly self-prompting now, but still single-track and still paced by the clock
...
This is the bit that matters most. Every rung up only pays off if the work hands something back I can trust: a verification gate. Tests that pass, a build that's green, a pull request I can glance at and believe.

Without that signal, autonomous output collapses straight back into manual re-review. The loop runs, hands me a pile of work, and I have to read all of it to know whether any of it's right – at which point I haven't saved the time, I've just moved it. So the higher I climb, the more the verification design is the work. It's the whole difference between a loop that looks impressive in a demo and one that actually takes something off my plate.
...
Step back from Claude Code for a moment, because the ladder isn't a coding thing. It's a general move, and code is just where I see it most clearly.
