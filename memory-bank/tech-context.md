...
- **`public/`** – served as-is, no processing. Use for assets referenced by URL string. Current assets: `favicon.png` and `apple-touch-icon.png`, generated by `pnpm generate:favicons` ("dh" monogram in Geist Mono on the teal accent – regenerate if those tokens change)
