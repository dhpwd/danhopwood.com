...
- Share links refactored to per-platform `buildHref` (see `tech-context.md` "Share links"). Fixed broken endpoints: X (`/intent/tweet`), WhatsApp (`api.whatsapp.com/send`), Facebook (canonical). URLs `encodeURIComponent`'d, `via=dhpwd` added on X
...
- Newsletter signup hardened for View Transitions (see `system-patterns.md`). Native POST fallback now shows honest "Complete signup in the new tab" message instead of false success
...
- Cross-links added in Techie, Cowork, and Source-code posts pointing to "the memory bank framework". Source-code post gets a reciprocal aside at the end of its "Five layers of context management" section
- Trailing-slash normalisation on internal links: all `/posts/...` references across five blog files stripped of trailing slashes, including anchor links. Matches site-wide config (see `tech-context.md` "Trailing slash")
